    ...
    <div class="bg-[var(--card)] border border-[var(--line)] rounded-[var(--radius)] overflow-hidden">
      ...
      <table v-else class="w-full text-sm">
        ...
        <tbody class="divide-y divide-[var(--line-soft)]">
          ...
          >
            ...
            <td class="px-5 py-3.5 text-xs text-[var(--ink-400)] hidden sm:table-cell whitespace-nowrap">
              {{ entry.relativeUpdated }}
            </td>
      ...
      <div v-if="!programmes.entriesLoading && programmes.lastPage > 1" class="flex items-center justify-between gap-3 p-3.5 px-5 border-t border-[var(--line)]">
        ...
        <div class="flex gap-0.5">
          <button
            class="min-w-[34px] h-[34px] inline-flex items-center justify-center px-2.5 rounded-lg border border-[var(--line)] bg-[var(--card)] text-[12.5px] font-semibold text-[var(--ink-600)] hover:border-[var(--teal-600)] hover:text-[var(--teal-700)] disabled:opacity-35 disabled:cursor-not-allowed"
            :disabled="programmes.currentPage === 1"
            @click="programmes.fetchEntries(programmes.currentPage - 1)"
          >‹ Prev</button>
          <button
            class="min-w-[34px] h-[34px] inline-flex items-center justify-center px-2.5 rounded-lg border border-[var(--line)] bg-[var(--card)] text-[12.5px] font-semibold text-[var(--ink-600)] hover:border-[var(--teal-600)] hover:text-[var(--teal-700)] disabled:opacity-35 disabled:cursor-not-allowed"
            :disabled="programmes.currentPage === programmes.lastPage"
            @click="programmes.fetchEntries(programmes.currentPage + 1)"
          >Next ›</button>
        </div>
      </div>
